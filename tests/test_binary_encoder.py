...
def test_differential_edge_values():
    ...
    cases = [
        # all zeros
        dict(killmail_ids=[0] * n, killmail_times=[0] * n, x=[0] * n,
             y=[0] * n, z=[0] * n, ship_types=[0] * n),
        # all identical (zero deltas after the first)
        dict(killmail_ids=[5] * n, killmail_times=[9] * n, x=[1] * n,
             y=[-1] * n, z=[0] * n, ship_types=[3] * n),
        # strictly descending large ids (negative deltas) + extreme +/- positions
        dict(killmail_ids=list(range(200_000_000_000, 200_000_000_000 - n, -1)),
             killmail_times=list(range(1_800_000_000, 1_800_000_000 - n, -1)),
             x=[10**17, -(10**17)] * (n // 2), y=[-(10**17), 10**17] * (n // 2),
             z=[0] * n, ship_types=[50_000, 0] * (n // 2)),
    ]
    for c in cases:
