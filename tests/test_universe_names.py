...
def test_universe_names_routes_by_range_and_probes_ambiguous(monkeypatch):
    ...
    async def fake_entity_names(
        char_ids, corp_ids, alliance_ids, faction_ids, **kwargs
    ):
    ...
    assert (
        150000000 in captured["chars"]
        and 150000000 in captured["corps"]
        and 150000000 in captured["alliances"]
    )
    assert out[587].category == "type" and out[587].image_url.endswith(
        "/types/587/icon?size=32"
    )
    assert out[500003].category == "faction" and out[500003].image_url.endswith(
        "/corporations/500003/logo?size=32"
    )
