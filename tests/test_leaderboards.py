...
def test_window_literal_matches_window_map():
    ...
    assert set(get_args(lb.Scope)) == set(lb.SCOPES)
    assert lb.SCOPES == ("all", "players")
    assert lb.WINDOWS == {
...
def test_fetch_leaderboards_query_shape_and_params(monkeypatch):
    ...
    asyncio.run(lb.fetch_leaderboards("7d", "attacker", "all", 10))
    assert "FROM entity_leaderboard" in fake.sql
    assert "AND scope = $4 AND rank <= $5 " in fake.sql
    assert "ORDER BY facet_kind, rank" in fake.sql
    assert fake.args == (
        [ATTRIBUTE_KINDS[k] for k in lb.KINDS],
        SIDE_ROLES["attacker"],
        "week",
        "all",
        10,
    ...
    asyncio.run(lb.fetch_leaderboards("all", "victim", "players", 3))
    assert fake.args == ([1, 2, 3, 4, 5, 6], 0, "all", "players", 3)
...
def test_fetch_leaderboards_assembles_boards(monkeypatch):
    ...
    out = asyncio.run(lb.fetch_leaderboards("7d", "attacker", "all", 10))
...
def test_fetch_leaderboards_keeps_rank_order_not_id_order(monkeypatch):
    ...
    monkeypatch.setattr(lb, "db", _FakeDb(rows))
    _patch_resolvers(monkeypatch)
    out = asyncio.run(lb.fetch_leaderboards("1d", "victim", "players", 10))
...
def test_fetch_leaderboards_empty_window(monkeypatch):
    ...
    _patch_resolvers(monkeypatch)
    out = asyncio.run(lb.fetch_leaderboards("1d", "victim", "all", 10))
...
def test_endpoint_serves_cache_hit_with_revalidate(monkeypatch):
    ...
    async def fake_get(prefix, params):
        ...
        assert params == {"window": "7d", "role": "attacker", "scope": "all", "limit": 10}
        return '"lb"', False, body

    monkeypatch.setattr(stats.query_cache, "get", fake_get)
    resp = asyncio.run(
        stats.get_leaderboards(
            window="7d", role="attacker", scope="all", limit=10, if_none_match=None
        )
    )
    ...
    resp = asyncio.run(
        stats.get_leaderboards(
            window="7d", role="attacker", scope="all", limit=10, if_none_match='"lb"'
        )
    )
...
def test_builder_single_flight_and_cache_params(monkeypatch):
    ...
    async def fake_fetch(window, role, scope, limit):
        calls.append((window, role, scope, limit))
    ...
    async def go():
        return await asyncio.gather(
            *[
                stats.get_leaderboards(
                    window="30d", role="victim", scope="players", limit=5, if_none_match=None
                )
                for _ in range(6)
            ]
        )
    ...
    assert calls == [("30d", "victim", "players", 5)]
...
def test_builder_caches_scopes_separately(monkeypatch):
    # The two scopes of one window/role/limit are different boards and must never
    # share a cache entry or a single-flight lock.
    store: dict = {}
    fetched: list = []

    async def fake_get(prefix, params):
        return store.get(json.dumps(params, sort_keys=True))

    async def fake_set(prefix, params, value, ttl=None):
        store[json.dumps(params, sort_keys=True)] = ('"e"', False, value.encode())
        return store[json.dumps(params, sort_keys=True)]

    async def fake_fetch(window, role, scope, limit):
        fetched.append(scope)
        return LeaderboardsResponse(
            character=[LeaderboardEntry(id=1 if scope == "all" else 2, kills=1)],
            corporation=[], alliance=[], faction=[], ship=[], weapon=[],
        )
    ...
    all_body = asyncio.run(stats.build_leaderboards("7d", "attacker", "all", 10))[2]
    players_body = asyncio.run(
        stats.build_leaderboards("7d", "attacker", "players", 10)
    )[2]
    assert fetched == ["all", "players"]
    assert all_body != players_body
    assert len(store) == 2
