...
def test_processed_cached_bytes_have_no_null_fields(monkeypatch):
    ...
    monkeypatch.setattr(kills.query_cache, "get", fake_get)
    monkeypatch.setattr(kills.query_cache, "set", fake_set)
    monkeypatch.setattr(kills, "fetch_kills_by_ids", fake_fetch)
    monkeypatch.setattr(kills.entities, "fetch_entity_names", fake_names)
    monkeypatch.setattr(kills, "get_type_names", fake_types)

    resp = asyncio.run(kills.get_kill_details_processed(9, if_none_match=None))
...
def test_processed_404_when_missing(monkeypatch):
    ...
    with pytest.raises(HTTPException) as exc:
        asyncio.run(kills.get_kill_details_processed(123, if_none_match=None))
...
def test_raw_details_304_on_matching_etag(monkeypatch):
    ...
    resp = asyncio.run(kills.get_kill_details("1", if_none_match=etag))
